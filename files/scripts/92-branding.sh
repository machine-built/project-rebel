...
NAME="${NAME:-schloss}"
WP_DESKTOP="${NAME_WP:-foss}"
WP_LOGIN="${NAME_WP_LOGIN:-SchlossLogin}"
...
for wp in "${WP_DESKTOP}" "${WP_LOGIN}"; do
    test -f "/usr/share/wallpapers/${wp}/metadata.json" \
        || { echo "ERROR: ${wp} is not a wallpaper package" >&2; exit 1; }
...
LNF_DEFAULTS="/usr/share/plasma/look-and-feel/${LNF_ID}/contents/defaults"
test -f "${LNF_DEFAULTS}" || { echo "ERROR: missing ${LNF_DEFAULTS}" >&2; exit 1; }
grep -qx "Image=${WP_DESKTOP}" "${LNF_DEFAULTS}" \
    || { echo "ERROR: LNF defaults does not name ${WP_DESKTOP}" >&2; exit 1; }
...
rpm -q plasma-login-manager || { echo "ERROR: Plasma LM removed" >&2; exit 1; }
ls -l "${PLM_DEFAULTS}" || true    # log whether an RPM shipped it, and its mode

# Lock screen
kwriteconfig6 --file "${KDEPROFILE}/kscreenlockerrc" "${WPGROUP[@]}" \
    --key Image "file:///usr/share/wallpapers/${WP_LOGIN}"
chmod 0644 "${KDEPROFILE}/kscreenlockerrc"
